...
{
	double PSNR = 0, t;
	int i, j,k,l,s,s2,r1,r2,r3,r4,s3,s4;
	...
	for (i=1; i<SIZE-1; i+=1 ) {
	  s=s+SIZE;
	      for (j=1; j<SIZE-1; j+=1) {
			...
			res =-input[s4 - 1]+ input[s4] * (-2)-input[s4 + 1]+ input[s3  - 1]+ input[s3 ] * 2+ input[s3 + 1] ;
							
			p1= res*res ; 
			res = 0;
			
			res =input[s4 - 1]-input[s4 + 1]+ input[s2 - 1] * 2+ input[s2+ 1] * (-2)+ input[s3  - 1]-input[s3 + 1] ;
			 ...
			 * to 255.											   */
			if (res > 255)
				PSNR+=(255 - golden[s2])*(255 - golden[s2]);
			else
				PSNR+=((unsigned char)res - golden[s2])*((unsigned char)res - golden[s2]);		
		}
	}
