...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1 ) {
	      for (j=1; j<SIZE-1; j+=2) {
			...
			res += input[(i - 1)*SIZE + j - 1] * (-1);
			res += input[(i - 1)*SIZE + j] * (-2);
			res += input[(i - 1)*SIZE + j + 1] * (-1);
			...
			res += input[(i - 1)*SIZE + j ] * (-1);
			res += input[(i - 1)*SIZE + j+1] * (-2);
			res += input[(i - 1)*SIZE + j + 2] * (-1);

			
			
			

			res += input[(i + 1)*SIZE + j ] ;
			res += input[(i + 1)*SIZE + j+1] * 2;
			res += input[(i + 1)*SIZE + j + 2] ;
			...
			res += input[(i)*SIZE + j ] * 2;
			
			res += input[(i)*SIZE + j + 2] * (-2);
